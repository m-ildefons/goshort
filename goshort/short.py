...
def redirect(name):
    ...
    row = database.execute(
        "SELECT long_uri " "FROM urls WHERE ident = ?", (name,)
    ).fetchone()
    ...
    if not row:
        ...
        golinks = database.execute(
            "SELECT u.* from urls u "
            "WHERE u.id IN "
            "(SELECT h.uri_id from home h)"
        ).fetchall()
...
def _insert(long_uri, ident):
    ...
    if long_uri:
        ...
        cursor = database.execute(
            "INSERT INTO urls (ident, long_uri) " "VALUES (?, ?)",
            (ident, long_uri),
        )
        if save_home:
            database.execute(
                "INSERT INTO home (display_name, uri_id) " "VALUES (?, ?)",
...
def _random_string(size=8, char_selection=string.ascii_lowercase):
    ...
    while (
        not _string
        or database.execute(
            "SELECT ident FROM urls " "WHERE ident = ?", (_string,)
        ).fetchall()
    ):
